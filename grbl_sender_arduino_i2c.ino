...
#define SpindleStartRelay A0
#define SpindleRelay      A1
#define yellowLED        8
#define greenLED         9

#define Msw12 3  // the switch at 12 o'clock
#define Msw3  4  // the switch at 3  o'clock
#define Msw6  5  // the switch at 6  o'clock
#define Msw9  6  // the switch at 9  o'clock
#define resetpin 7
...
//  lcd.begin(20, 4);
  ...
  setTextDisplay(F("Rangoli Machine"), "        BY","TECK TEAM SOLUTIONS", F("         Connect?-->"));
  //  delay(2000);
...
void emergencyBreak() {
  ...
  while (true) {
    if (!digitalRead(Msw3)) {
      Serial.print("~");  // send continue and return
      return;
    }
    if (!digitalRead(Msw6)) {
...
//    yval = analogRead(joystick_yPin);
    ...
    if (fileindex < fc && digitalRead(Msw6) == LOW) { // joystick down!
      fileindex++;
      fn = getFileName(fileindex);
      lcd.setCursor(0, 1);
      lcd.print(F("-> ")); lcd.print(fn);
//      for (int u = fn.length() + 8; u < 20; u++) {
//        lcd.print(" ");
//      }
    ...
    if (digitalRead(Msw12) == LOW) { // joystick up!
      if (fileindex > 1) {
        fileindex--;
        fn = "";
        fn = getFileName(fileindex);
...
//        }
    ...
    if (digitalRead(Msw3) == LOW) { // full right!
      waitForJoystickMid();
      return 0;
      setTextDisplay(F("Files "), "-> " + fn, "", F("Click to select"));
    }
...
void waitForJoystickMid() {
  ...
  //    xval = analogRead(joystick_xPin);
  //    yval = analogRead(joystick_yPin);
  //    if ((xval >= 500 && xval <= 600) && (yval >= 500 && yval <= 600)) {
  //      break;
  //    }
  //  }
  while (true) { // wait for the joystick to be release (back to mid position)
    if (digitalRead(Msw12) == HIGH && digitalRead(Msw6) == HIGH && digitalRead(Msw9) == HIGH && digitalRead(Msw3) == HIGH) {
      break;
    }
  }
}
...
void moveMenu() {
  ...
  */
  ...
  lcd.clear();
  String MoveCommand;
  bool up, down, left, right, updateDisplay;
  int xval, yval, qtime;
...
//    xval = analogRead(joystick_xPin);
//    yval = analogRead(joystick_yPin);
    up   = !digitalRead(Msw12);
    left = !digitalRead(Msw9);
    right   = !digitalRead(Msw3);
    down = !digitalRead(Msw6);
...
//    } else {
//      if (xval < 300) {
//        MoveCommand = F("G1 Y-1 F500"); // slow in reverse
//      } else {
//        if (xval > 900) {
...
//    }
    ...
    if (up)  {
      MoveCommand = F("G1 Y1 F200"); // UP
    }
    if (down)  {
      MoveCommand = F("G1 Y-1 F200"); // DOWN
    }
    if (left) {
      MoveCommand = F("G1 X-1 F200"); // LEFT
    }
    if (right) {
      MoveCommand = F("G1 X1 F200"); // RIGHT
    }
...
void updateDisplayStatus(unsigned long runtime) {
  ...
  */
  ...
  lcd.setCursor(0, 3);
  lcd.print(F("Z position: "));  lcd.print(WposZ); lcd.print(F("  "));
...
//  lineOfCode.replace(F("M2 "), "M5 M2 "); // Shut down spindle on program end.
  ...
  if (lineOfCode.startsWith("(") || lineOfCode.startsWith("T") ) {
    lineOfCode = "";
  }
  lineOfCode.trim();
  return lineOfCode;
...
//  Serial.println(content);
  ...
  c = 0;
  while (c < 8 && content[i] != ',') {
    MposX[c++] = content[i++];  // get MposX
    MposX[c] = 0;
  }
  c = 0; i++;
  while (c < 8 && content[i] != ',') {
    MposY[c++] = content[i++];  // get MposY
    MposY[c] = 0;
  }
  c = 0; i++;
  while (c < 8 && content[i] != ',') {
    MposZ[c++] = content[i++];  // get MposZ
    MposZ[c] = 0;
  }
